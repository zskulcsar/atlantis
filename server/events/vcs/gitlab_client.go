...
func (g *GitlabClient) DownloadRepoConfigFile(pull models.PullRequest, atlantisYAMLFilename string) (bool, []byte, error) {
	opt := gitlab.GetRawFileOptions{Ref: gitlab.String(pull.HeadBranch)}

	bytes, resp, err := g.Client.RepositoryFiles.GetRawFile(pull.BaseRepo.FullName, atlantisYAMLFilename, &opt)
